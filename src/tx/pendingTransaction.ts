...
class PendingTransaction {
    ...
    constructor(transaction: Promise<ICreateTransactions>) {
    ...
    public static createTransactionsWithEntries(
        ...
        const promise = PendingTransaction.createRawTransactions(entries, outputs, address, fee, priorityEntries, sigOpCount);
        return new PendingTransaction(promise);
    }
    ...
    public static createTransactions(
        ...
        const promise = (async () => {
            ...
            return PendingTransaction.createRawTransactions(entries, outputs, address, fee, priorityEntries, sigOpCount);
        })();
        return new PendingTransaction(promise);
    }
    ...
    public async submit(): Promise<string | undefined> {
        ...
        return this.transaction.then(async transactions => {
            for (const transaction of transactions.transactions) {
                await transaction.submit(client);
            }
            return transactions.summary.finalTransactionId;
        });
    }
}
